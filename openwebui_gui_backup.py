...
# Costanti di layout (H4.2 - Consistenza e standard)
SPACING_LARGE = 15
SPACING_MEDIUM = 10
SPACING_SMALL = 6
...
class DashboardWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # Stato servizi (H2.2 - Etichette orientate all'utente)
        status_group = QGroupBox("I tuoi servizi")
        ...
        # Azioni rapide (H2.2 - Etichette orientate all'utente)
        actions_group = QGroupBox("Cosa vuoi fare?")
        ...
        self.btn_stop.setToolTip("Ferma tutti i container Docker (Ctrl+S)")
...
class LogsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # H9.2 - Pulsante "Copia per Supporto"
        btn_copy_support = ModernButton("📋 Copia per Supporto", "purple", compact=True)
    ...
    def copy_for_support(self):
        """H9.2 - Copia log + info sistema formattato per issue GitHub"""
        ...
        self.main_window.statusBar().showMessage("Log copiato negli appunti!", 3000)
...
class ModelsWidget(QWidget):
    ...
    def _models_context_menu(self, pos):
        ...
        menu = QMenu(self)
        copy_action = menu.addAction("Copia nome")
        menu.addSeparator()
        remove_action = menu.addAction("Rimuovi modello")

        action = menu.exec_(self.models_list.mapToGlobal(pos))
        if action == copy_action:
            QApplication.clipboard().setText(model_name)
            self.main_window.statusBar().showMessage(f"Copiato: {model_name}", 2000)
        elif action == remove_action:
            msg = QMessageBox(self)
            msg.setIcon(QMessageBox.Warning)
            msg.setWindowTitle("Conferma Rimozione")
            msg.setText(f"Vuoi rimuovere il modello '{model_name}'?")
    ...
    def download_model(self):
        ...
        if not model:
            ...
            self.main_window.statusBar().showMessage("Inserisci il nome di un modello", 3000)
...
class MCPWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        warning_label = QLabel("⚠️ <b style='color: #856404;'>MCP richiede risorse.</b> <span style='color: #856404; font-size: 10px;'>Verifica RAM/VRAM prima di avviare.</span>")
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # H2.1 - Tab riordinati per workflow naturale
        self.tabs.addTab(self.dashboard, "🏠 Dashboard")       # 0
        self.tabs.addTab(self.models, "🤖 Modelli")             # 1
        self.tabs.addTab(self.tts_widget, "🔊 Voce")            # 2
        self.tabs.addTab(self.archivio, "📁 Archivio")          # 3
        self.tabs.addTab(self.mcp_widget, "🔌 MCP")             # 4
        self.tabs.addTab(self.config, "⚙️ Configurazione")      # 5
        self.tabs.addTab(self.logs, "📜 Log")                    # 6
        self.tabs.addTab(self.info_widget, "ℹ️ Informazioni")   # 7

        layout.addWidget(self.tabs)

        # H10.1 - Pulsante "?" help contestuale per ogni tab
        self._tab_help = {
            0: ("Dashboard", "Panoramica dello stato dei servizi.\n\nDa qui puoi avviare, fermare o riavviare Docker e Open WebUI,\ne monitorare le risorse del sistema."),
            1: ("Modelli", "Gestisci i modelli AI di Ollama.\n\nScarica nuovi modelli dalla tabella consigliati\no inserisci manualmente il nome.\nClicca sul nome del modello per scaricarlo."),
            2: ("Voce", "Sintesi vocale italiana offline con Piper TTS.\n\nConfigura e testa le voci italiane.\nScarica le voci se non ancora installate."),
            3: ("Archivio", "Gestisci file locali da usare come archivio documenti.\n\nTrascina o aggiungi file che verranno indicizzati\nda Open WebUI per le risposte."),
            4: ("MCP", "Model Context Protocol Bridge.\n\nEspone i servizi locali (TTS, Image, Document)\ntramite protocollo MCP per integrazioni esterne."),
            5: ("Configurazione", "Impostazioni avanzate.\n\nAccesso LAN, HTTPS, lingua italiana,\nmanutenzione e backup."),
            6: ("Log", "Log dei container Docker in tempo reale.\n\nUtile per diagnosticare problemi.\nUsa 'Segui Log' per lo streaming continuo."),
            7: ("Informazioni", "Informazioni sul progetto,\nversione e crediti."),
        }
        help_btn = QPushButton("?")
        help_btn.setFixedSize(28, 28)
        help_btn.setCursor(Qt.PointingHandCursor)
        help_btn.setToolTip("Aiuto per questo tab (F1 per guida completa)")
        ...
        """)
        help_btn.clicked.connect(self._show_tab_help)
        ...
        # H7.1 - Nuove shortcut tastiera
        quit_shortcut = QShortcut(QKeySequence("Ctrl+Q"), self)
        quit_shortcut.activated.connect(self.confirm_and_quit)

        dark_shortcut = QShortcut(QKeySequence("Ctrl+D"), self)
        dark_shortcut.activated.connect(lambda: self.dark_mode_btn.click())

        f5_shortcut = QShortcut(QKeySequence("F5"), self)
        f5_shortcut.activated.connect(self.dashboard.update_system_info)

        f1_shortcut = QShortcut(QKeySequence("F1"), self)
        f1_shortcut.activated.connect(self.show_global_help)
        ...
        # === SINISTRA: Mini indicatori servizi (H6.1) ===
        ...
        # Status bar + pulsante Annulla (H3.1) + timer elapsed (H1.1)
    ...
    def setup_tray(self):
        ...
        quit_action.triggered.connect(self.confirm_and_quit)
    ...
    def update_status(self, status):
        ...
        # H5.2 - Pulsanti smart in base a stato servizi (solo se nessuna operazione in corso)
        ...
        # Aggiorna status bar solo se non c'è un'operazione in corso
        if not self._operation_active:
    ...
    def _set_operation_in_progress(self, active):
        """H5.1 - Disabilita/abilita pulsanti durante operazioni"""
    ...
    def command_finished(self, code):
        ...
        if code == -1:
            # Annullato
            self.statusBar().showMessage(f"Annullato ({elapsed}s)")
            self.statusBar().setStyleSheet("background-color: #ffeaa7;")
            self.logs.append_log(f"\n⚠ Annullato dopo {elapsed}s")
        elif code == 0:
            self.statusBar().showMessage(f"Completato con successo ({elapsed}s)")
            self.statusBar().setStyleSheet("background-color: #d4edda;")
            self.logs.append_log(f"\n✓ Completato in {elapsed}s")
        else:
            self.statusBar().showMessage(f"Errore (codice {code}, {elapsed}s)")
            self.statusBar().setStyleSheet("background-color: #fadbd8;")
            self.logs.append_log(f"\n✗ Errore (codice {code}) dopo {elapsed}s")
            # H9.1 - Dialog errore con suggerimenti
            self._show_error_dialog(code)

        # H1.3 - Reset colore status bar dopo timeout
        reset_delay = 5000 if code == 0 else 10000
        QTimer.singleShot(reset_delay, lambda: self.statusBar().setStyleSheet(""))

        # H1.2 - Notifica desktop se finestra non visibile
        if not self.isVisible() or self.isMinimized():
            title = "Operazione completata" if code == 0 else "Errore operazione"
            msg = f"{'Successo' if code == 0 else f'Errore (codice {code})'} - {elapsed}s"
    ...
    def _show_error_dialog(self, code):
        """H9.1 - Dialog errore con suggerimenti contestuali e Riprova"""
        cmd = getattr(self, '_last_command', '')
        suggestions = []
        if 'docker' in cmd:
            suggestions = [
                "Verifica che Docker sia in esecuzione",
                "Prova a riavviare Docker Desktop",
            ...
            suggestions = [
                ...
                "Il nome del modello potrebbe essere errato"
            ]
        elif 'ollama rm' in cmd:
            suggestions = [
                "Il modello potrebbe essere gia' stato rimosso",
                "Verifica che Ollama sia in esecuzione"
            ]
        else:
            suggestions = [
        ...
        text = f"L'operazione e' terminata con codice errore {code}.\n\nSuggerimenti:\n"
        for s in suggestions:
            text += f"  - {s}\n"

        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Warning)
        msg.setWindowTitle("Operazione non riuscita")
        msg.setText(text)

        retry_btn = msg.addButton("Riprova", QMessageBox.AcceptRole)
        log_btn = msg.addButton("Vedi Log", QMessageBox.ActionRole)
    ...
    def retranslate_ui(self):
        ...
        # Tab names (H2.1 - ordine aggiornato)
    ...
    def _check_first_run(self):
        ...
        msg = QMessageBox(self)
        msg.setWindowTitle("Benvenuto in Open WebUI Manager!")
        msg.setTextFormat(Qt.RichText)
        msg.setText(
            "<h3>Benvenuto!</h3>"
            "<p>Per iniziare, segui questi 3 passi:</p>"
            "<ol>"
            "<li><b>Modelli</b> - Scarica almeno un modello AI (consigliato: qwen2.5:7b)</li>"
            "<li><b>Voce</b> - Configura la sintesi vocale italiana (opzionale)</li>"
            "<li><b>Browser</b> - Apri Open WebUI e inizia a chattare!</li>"
            "</ol>"
            "<p><small>Premi OK per andare al tab Modelli.</small></p>"
        )
        dont_show = QCheckBox("Non mostrare piu'")
        ...
        # Naviga al tab Modelli
        self.tabs.setCurrentIndex(1)

    def _show_tab_help(self):
        """H10.1 - Mostra help contestuale per il tab corrente"""
        idx = self.tabs.currentIndex()
        title, text = self._tab_help.get(idx, ("Help", "Nessun aiuto disponibile."))
        QMessageBox.information(self, f"Guida - {title}", text)

    def confirm_and_quit(self):
        """H3.2 - Warning alla chiusura con servizi attivi"""
        # Check rapido servizi
        services_active = False
        status = getattr(self, '_last_service_status', {})
        if status.get('ollama') or status.get('openwebui'):
            services_active = True

        if services_active:
            msg = QMessageBox(self)
            msg.setIcon(QMessageBox.Question)
            msg.setWindowTitle("Chiudi Open WebUI Manager")
            msg.setText("Ci sono servizi ancora attivi.\nCosa vuoi fare?")

            close_btn = msg.addButton("Chiudi Manager", QMessageBox.AcceptRole)
            stop_btn = msg.addButton("Ferma tutto e chiudi", QMessageBox.DestructiveRole)
    ...
    def show_global_help(self):
        """H10.2 - F1 Help globale"""
        help_html = """
        <h2>Open WebUI Manager - Guida Rapida</h2>

        <h3>Scorciatoie Tastiera</h3>
        <table style='border-collapse: collapse; width: 100%;'>
        <tr><td style='padding: 4px;'><b>Ctrl+1..8</b></td><td>Naviga tra i tab</td></tr>
        <tr><td style='padding: 4px;'><b>Ctrl+R / F5</b></td><td>Aggiorna stato servizi</td></tr>
        <tr><td style='padding: 4px;'><b>Ctrl+B</b></td><td>Apri browser Open WebUI</td></tr>
        <tr><td style='padding: 4px;'><b>Ctrl+D</b></td><td>Dark/Light mode</td></tr>
        <tr><td style='padding: 4px;'><b>Ctrl+Q</b></td><td>Esci</td></tr>
        <tr><td style='padding: 4px;'><b>F1</b></td><td>Questa guida</td></tr>
        </table>

        <h3>Primo Avvio</h3>
        <ol>
        <li>Scarica un modello dal tab <b>Modelli</b></li>
        <li>Avvia i servizi dalla <b>Dashboard</b></li>
        <li>Apri il browser per usare Open WebUI</li>
        </ol>

        <h3>Link Utili</h3>
        <ul>
        <li>Open WebUI: <a href='http://localhost:3000'>http://localhost:3000</a></li>
        <li>Ollama: <a href='http://localhost:11434'>http://localhost:11434</a></li>
        </ul>
        """
        msg = QMessageBox(self)
        msg.setWindowTitle("Guida - Open WebUI Manager")
        msg.setTextFormat(Qt.RichText)
